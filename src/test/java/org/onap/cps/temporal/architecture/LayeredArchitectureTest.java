...
public class LayeredArchitectureTest {
    ...
    @ArchTest
    static final ArchRule controllerAccessRule =
            classes().that().resideInAPackage(CONTROLLER_PACKAGE)
                    .should().onlyBeAccessed().byAnyPackage(CONTROLLER_PACKAGE);

    @ArchTest
    static final ArchRule serviceAccessRule =
            classes().that().resideInAPackage(SERVICE_PACKAGE)
                    .should().onlyBeAccessed().byAnyPackage(CONTROLLER_PACKAGE, SERVICE_PACKAGE);

    @ArchTest
    static final ArchRule repositoryAccessRule =
            classes().that().resideInAPackage(REPOSITORY_PACKAGE)
                    .should().onlyBeAccessed().byAnyPackage(SERVICE_PACKAGE, REPOSITORY_PACKAGE);
