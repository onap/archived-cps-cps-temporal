...
class NetworkDataRepositoryImplSpec extends Specification {
    ...
    static final String RELOAD_DATA_FOR_SEARCHING = '/data/reload-network-data.sql'
    ...
    def 'Query: pagination for #scenario'() {
        ...
        and: ' correct pagination details'
            result.hasNext() ? result.nextPageable() : null == nextPageable
            result.hasPrevious() ? result.previousPageable() : null == previousPageable
        where:
            scenario      | pageNumber | nextPageable         | previousPageable
            'first Page'  | 0          | PageRequest.of(1, 1) | null
            'middle Page' | 1          | PageRequest.of(2, 1) | PageRequest.of(0, 1)
            'last Page'   | 2          | null                 | PageRequest.of(1, 1)
    }
