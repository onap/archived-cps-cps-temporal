...
class DataUpdatedEventListenerIntegrationSpec extends Specification {
    ...
    def sqlQuerySuffix =
            "from network_data " +
                    ...
                    "and schema_set = ? " +
                    "and anchor = ?"
    def sqlCount = "select count(*) " + sqlQuerySuffix
    def sqlStar =  "select * " + sqlQuerySuffix
    ...
    def 'Processing a valid event'() {
        given: "no event has been proceeded"
            assert (jdbcTemplate.queryForObject(
                    sqlCount, Integer.class, timestampStr, aDataspace, aSchemaSet, anAnchor) == 0)
        when: 'an event is produced'
            this.kafkaTemplate.send(topic, event)
        then: 'the event is proceeded'
            condition.eventually {
                assert (jdbcTemplate.queryForObject(
                        sqlCount, Integer.class, timestampStr, aDataspace, aSchemaSet, anAnchor) == 1)
            }
            Map<String, Object> result =
                    jdbcTemplate.queryForMap(sqlStar, timestampStr, aDataspace, aSchemaSet, anAnchor)
    ...
    def 'Processing an invalid event'() {
        ...
        then: 'no processing is done (all error details are still logged)'
            Thread.sleep(1000)
    }
