...
class DataUpdatedEventListenerIntegrationSpec extends Specification {
    ...
    def 'Processing a valid event'() {
        ...
        given: 'no data exist for the anchor'
            assert !dataExists(aTimestamp, aDataspace, aSchemaSet, anAnchor)
    ...
    def dataExists(observedTimestamp, dataspaceName, schemaSetName, anchorName) {
        def recordCount = jdbcTemplate.queryForObject(
                sqlCountWithConditions, Integer.class, observedTimestamp, dataspaceName, schemaSetName, anchorName)
