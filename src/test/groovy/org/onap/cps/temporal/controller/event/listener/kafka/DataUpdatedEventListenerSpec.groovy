...
class DataUpdatedEventListenerSpec extends Specification {
    ...
    def objectUnderTest = new DataUpdatedEventListener(mockService)
            ...
            1 * mockService.addNetworkData(
                    {
                        ...
                        && it.getPayload() == '{"' + aDataName + '":"' + aDataValue + '"}'
        ...
        then: 'an exception is thrown'
            ...
            e.getInvalidFields().contains(
                    new InvalidEventEnvelopException.InvalidField(
                            InvalidEventEnvelopException.Type.MISSING,
                            "schema", null,
                            CpsDataUpdatedEvent.Schema.URN_CPS_ORG_ONAP_CPS_DATA_UPDATED_EVENT_SCHEMA_1_1_0_SNAPSHOT
                                    .value()))
            e.getInvalidFields().contains(
                    new InvalidEventEnvelopException.InvalidField(
                            InvalidEventEnvelopException.Type.MISSING, "id", null, null))
            e.getInvalidFields().contains(
                    new InvalidEventEnvelopException.InvalidField(
                            InvalidEventEnvelopException.Type.UNEXPECTED, "source", null, eventSource.toString()))
            e.getInvalidFields().contains(
                    new InvalidEventEnvelopException.InvalidField(
                            InvalidEventEnvelopException.Type.UNEXPECTED, "type", null, eventType))
    }

    def 'Event message consumption fails because on invalid envelop'() {
        ...
        then: 'an exception is thrown'
            ...
            e.getInvalidFields().contains(
                    new InvalidEventEnvelopException.InvalidField(
                            InvalidEventEnvelopException.Type.UNEXPECTED, "type", aType, eventType))
            e.getInvalidFields().contains(
                    new InvalidEventEnvelopException.InvalidField(
                            InvalidEventEnvelopException.Type.UNEXPECTED,
                            "source", aSource.toString(), eventSource.toString()))
