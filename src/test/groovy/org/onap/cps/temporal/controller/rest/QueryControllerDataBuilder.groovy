...
class QueryControllerDataBuilder {
    ...
    SearchCriteria.Builder createSearchCriteriaBuilder() {
        ...
        if (parameters.observedTimestampAfter != null)
            searchCriteriaBuilder.observedAfter(DateTimeUtility.getOffsetDateTime(parameters.observedTimestampAfter))
        if (parameters.pageNumber != null)
    ...
    private String createNextExpectedLink() {
        def pageNumber = getPageNumber()
        return getLink(getPageNumber() + 1)
    }

    private String createPreviousExpectedLink() {
        return getLink(getPageNumber() - 1)
    }
    ...
    private String getLink(int pageNumber) {
        ...
        if (parameters.observedTimestampAfter != null)
            uriComponentBuilder.queryParam(OBSERVED_TIMESTAMP_AFTER_QUERY_PARAM, parameters.observedTimestampAfter)
        if (parameters.pointInTime != null)
    ...
    boolean isExpectedPreviousRecordsLink(String actualPreviousLink) {
        ...
        createPreviousExpectedLink() == actualPreviousLink
    }
