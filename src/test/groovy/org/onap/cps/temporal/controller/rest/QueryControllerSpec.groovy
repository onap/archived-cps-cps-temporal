...
// TODO check if can split it into multiple classes
...
class QueryControllerSpec extends Specification {
    ...
    // TODO too many items being tested in one test case
    def 'Get #endpointName: query data with provided values'() {
        ...
        and: 'next and previous record links have expected value'
            responseBody.getNextRecordsLink() == controllerDataBuilder.createNextExpectedLink()
            responseBody.getPreviousRecordsLink() == controllerDataBuilder.createPreviousExpectedLink()
        and: 'has expected network data records'
            responseBody.getRecords().size() == 1
            responseBody.getRecords() == [toAnchorDetails(networkData)]
        where:
            endpointName           | urlSpecifParams          | observedTimestamp              | payloadFilter
            'anchor by name'       | [anchor: myAnchor]       | null                           | null
            'anchor by name'       | [anchor: myAnchor]       | '2021-07-24T03:00:01.000-0400' | null
            'anchor by name'       | [anchor: myAnchor]       | null                           | '{"message" : "hello world"}'
            'anchor by name'       | [anchor: myAnchor]       | '2021-07-24T03:00:01.000-0400' | '{"message" : "hello world"}'
            'anchors by schemaset' | [schemaSet: mySchemaset] | null                           | null
            'anchors by schemaset' | [schemaSet: mySchemaset] | '2021-07-24T03:00:01.000-0400' | null
            'anchors by schemaset' | [schemaSet: mySchemaset] | null                           | '{"message" : "hello world"}'
            'anchors by schemaset' | [schemaSet: mySchemaset] | '2021-07-24T03:00:01.000-0400' | '{"message" : "hello world"}'
    }
    ...
    def 'Get #endpointName: Sort by #sortAsString'() {
        ...
        where:
            ...
            'anchor by name'       | [anchor: myAnchor]       | 'anchor:asc'                         || Sort.by(anchorAscSortOrder)
            ...
            'anchors by schemaset' | [schemaSet: mySchemaset] | 'anchor:asc'                         || Sort.by(anchorAscSortOrder)
    ...
    def 'Get #endpointName Error handling: invalid date format in #queryParamName '() {
        when: 'endpoint is called'
            def response = mvc.perform(get(endpoint)
                .queryParam(queryParamName, "invalid-date-string")
    ...
    def 'Get #endpointName Error handling: invalid simple payload filter '() {
        when: 'endpoint is called'
            def response = mvc.perform(get(endpoint)
                .queryParam('simplePayloadFilter', "invalid-json")
                .contentType(MediaType.APPLICATION_JSON))
                .andReturn().response
        then: 'received bad request status'
            response.getStatus() == HttpStatus.BAD_REQUEST.value()
