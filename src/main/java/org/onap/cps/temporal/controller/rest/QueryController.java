...
import org.onap.cps.temporal.controller.rest.api.CpsTemporalQueryApi;
...
public class QueryController implements CpsTemporalQueryApi {
    ...
    public ResponseEntity<AnchorHistory> getAnchorsDataByFilter(final String dataspaceName,
        final @NotNull @Valid String schemasetName, final @Valid String after,
        final @Valid String simplePayloadFilter, final @Valid String pointInTime,
        final @Min(0) @Valid Integer pageNumber, final @Min(0) @Max(10000) @Valid Integer pageLimit,
        final @Valid String sort) {
        return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
    }
}
